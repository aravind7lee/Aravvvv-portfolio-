...
export default function SelectionScreen({ onSelect }: SelectionScreenProps) {
  ...
  useEffect(() => {
    ...
    const hoverAudio = new Audio(gtaMenuHoverMp3);
    const selectAudio = new Audio(gtaRespectMp3);
  ...
  const playSanAndreasHoverFX = (_isRed: boolean) => {
    ...
    try {
      ...
      // Web Audio API GTA SA Menu Hover Sound (Guaranteed immediate response)
      const ctx = getAudioCtx();
      if (ctx) {
